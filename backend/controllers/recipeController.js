...
const getRecipeByID = async (req,res) => {
  ...
  try {
    let db = await connectToDatabase(process.env.DB_NAME);
    const fieldsToRetrieve = {id:1,title:1,servings:1,readyInMinutes:1,image:1,dishTypes:1,diets:1,summary:1,cuisines:1,extendedIngredients:1,analyzedInstructions:1}
    let recipe = await db.collection("recipes").findOne({ id: recipeID }, { projection: fieldsToRetrieve });
    console.log(recipe)
    res.json(recipe);
